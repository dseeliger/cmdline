...
#
# ----------------------------------------------------------------------
# pmx is Copyright (C) 2006-2015 by Daniel Seeliger
#
#                        All Rights Reserved
#
...
__doc__ = """Classes for commandline parsing inspired by the gromacs style commandline handling"""
...
class Option(OptionBase):
    ...
    def __error(self, arg):
        raise CmdlineError("Option \"%s\" (%s) is not compatible with argument ->" % (self.flag, self.type), arg)
...
class Commandline:
    ...
    def __init__(self, cmdline, options = [], fileoptions = [], program_desc = [] , check_for_existing_files = False, version = "1.0", verbose = True):
    ...
    def __consistency_check( self ):
        ...
        for opt in self.options+self.fileoptions:
            if opt.flag in olist:
                raise CmdlineError("Option flag \"%s\" defined multiple times" % opt.flag)
    ...
    def __check_flags(self):
        for flag in self.flag_list:
            ...
            if n != 1:
                raise CmdlineError("Flag \"%s\" appears %d times in commandline" %(flag, n))
            
    def __check_for_unparsed_args(self):
        for i, arg in enumerate(self.cmdline):
            if i not in self.parsed_opts:
                raise CmdlineError("Unknown argument \"%s\" in commandline" %(arg))
...
if __name__=='__main__':          
    ...
    files = [
        FileOption("-i", "r",["doc","docx"],"xx.doc", "input word files"),
        FileOption("-excel", "r/m",["xls","xlsx"],"", "one or more excel files"),
